...
Сконфигурировать `.env` файл, указать приставку и название пользователя в Бд, такого же пользователя для db нужно прописать в `mysqlconf/init/01-databases.sql` и можно изменить тестовую базу (в зависимости от того как она будет называться в проекте)
